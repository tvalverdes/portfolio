export const Bio = {
  name: 'Tyrone Valverde',
  roles: ['Front', 'Back'],
  description:
    'Apasionado por la tecnología y el desarrollo de software, con experiencia en el desarrollo de aplicaciones web. Me gusta aprender nuevas tecnologías y trabajar en equipo para lograr los objetivos propuestos.',
  github: 'https://github.com/tvalverdes',
  resume:
    'https://drive.google.com/file/d/13olZ74G29s1P4Q0rhIDukke0Lpx4yWkY/view?usp=sharing',
  linkedin: 'https://www.linkedin.com/in/tvalverdes',
}

export const skills = [
  {
    title: 'Frontend',
    skills: [
      {
        name: 'React Js',
        image:
          'data:image/svg+xml;base64,',
      },
      {
        name: 'Next Js',
        image:
          'data:image/png;base64,',
      },
      {
        name: 'HTML',
        image: 'https://www.w3.org/html/logo/badge/html5-badge-h-solo.png',
      },
      {
        name: 'CSS',
        image:
          'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png',
      },
      {
        name: 'JavaScript',
        image:
          'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png',
      },
      {
        name: 'Tailwind',
        image: 'https://files.raycast.com/sjxs3pxsc6k63ju0fzv8l3cu4v90',
      },
      {
        name: 'Bootstrap',
        image:
          'https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png',
      },
      {
        name: 'Material UI',
        image:
          'data:image/png;base64,',
      },
    ],
  },
  {
    title: 'Backend',
    skills: [
      {
        name: 'Node Js',
        image: 'https://nodejs.org/static/images/logo.svg',
      },
      {
        name: 'Express Js',
        image:
          'data:image/png;base64,',
      },
      {
        name: 'Java',
        image:
          'data:image/png;base64,',
      },
      {
        name: 'MySQL',
        image:
          'https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg',
      },
      {
        name: 'Postgresql',
        image: 'https://www.postgresql.org/media/img/about/press/elephant.png',
      },
      {
        name: 'MongoDB',
        image:
          'https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg',
      },
    ],
  },
  {
    title: 'Otros',
    skills: [
      {
        name: 'Git',
        image:
          'https://camo.githubusercontent.com/fbfcb9e3dc648adc93bef37c718db16c52f617ad055a26de6dc3c21865c3321d/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f6769742d73636d2f6769742d73636d2d69636f6e2e737667',
      },
      {
        name: 'GitHub',
        image:
          'https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png',
      },
      {
        name: 'VS Code',
        image:
          'https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519',
      },
      {
        name: 'Postman',
        image:
          'https://camo.githubusercontent.com/93b32389bf746009ca2370de7fe06c3b5146f4c99d99df65994f9ced0ba41685/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f676574706f73746d616e2f676574706f73746d616e2d69636f6e2e737667',
      },
      {
        name: 'Adobe XD',
        image:
          'https://camo.githubusercontent.com/c205ecbe12500177d102169d97bc1c17c545155fdf5ec78c08d54ac53e5b38c1/68747470733a2f2f63646e2e776f726c64766563746f726c6f676f2e636f6d2f6c6f676f732f61646f62652d78642e737667',
      },
      {
        name: 'Figma',
        image:
          'https://camo.githubusercontent.com/ed93c2b000a76ceaad1503e7eb9356591b885227e82a36a005b9d3498b303ba5/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f6669676d612f6669676d612d69636f6e2e737667',
      },
      {
        name: 'Bizagi',
        image:
          'data:image/png;base64,',
      },
    ],
  },
]

export const experiences = [
  {
    id: 0,
    role: 'Desarrollador de Software',
    company: 'Inversiones Nashimat S. A. C',
    date: 'Abril - Julio 2023',
    desc: 'Automatización de Registros y Mejora en la Gestión de la Información',
    skills: ['PHP', 'FTP', 'Figma', 'Bootstrap'],
  },
  {
    id: 1,
    role: 'Desarrollado Web',
    company: 'Pasión por las Finanzas',
    date: 'Noviembre 2021 - Marzo 2023',
    desc: 'Creación de web para empresa del sector financiero.',
    skills: [
      'ReactJS',
      'NextJS',
      'Material UI',
      'HTML',
      'CSS',
      'JavaScript',
      'Wordpress',
    ],
  },
  {
    id: 2,
    role: 'Desarrollador Web',
    company: 'Transportes Valso',
    date: 'Agosto 2022',
    desc: 'Construcción de un sitio web para una de transporte de carga.',
    skills: ['Wordpress'],
  },
]

export const education = [
  {
    id: 0,
    school: 'Universidad Privada Antenor Orrego',
    date: '2016 - 2023',
    grade: 'Bachiller',
    desc: 'Ingeniero de Computación y Sistemas, en la Universidad Privada Antenor Orrego, donde he aprendido a desarrollar software de calidad, con las mejores prácticas y metodologías de desarrollo. Además, aprendí a trabajar en equipo y con metodologías ágiles',
    degree: 'Ingeniería de Computación y Sistemas',
  },
  {
    id: 1,
    school: 'Oracle & Alura',
    date: 'Marzo - Septiembre 2023',
    desc: 'Formé parte del equipo de desarrolladores backend del programa Oracle Next Education F2 T5 Back-end',
  },
]

export const projects = [
  {
    id: 0,
    title: 'GameNext',
    date: 'Feb 2023',
    description:
      'Web creada para visualización de información relacionada con videojuegos, permitiendo buscar por nombre y apartado de recomendaciones según categoría; consumiendo una API REST de Rapid API',
    image: '/gamenext.webp',
    tags: ['Vercel', 'Eslint', 'NodeJs', 'NextJs', 'React Js', 'Tailwind CSS'],
    category: 'web app',
    github: 'https://github.com/tvalverdes/gamenext',
    webapp: 'https://gamenext.tvalverde.tech',
  },
  {
    id: 1,
    title: 'Notly',
    date: 'Oct 2023',
    description:
      'Web creada para el almacenamiento de notas, permitiendo crear, editar y eliminar notas. Con autenticación de usuario para proteger y segmentar las notas. Utilicé MongoDB como base de datos, React para el desarrollo front end y NodeJS|Express para el back end.',
    image: 'notly.webp',
    tags: [
      'ReactJS',
      'Chakra UI',
      'NodeJS',
      'Express',
      'Tailwind CSS',
      'MongoDB',
      'JWT',
      'Redux',
    ],
    category: 'web app',
    github: 'https://github.com/stars/tvalverdes/lists/notes',
    webapp: 'https://notly.tvalverde.tech',
  },
  {
    id: 2,
    title: 'Pasión por las Finanzas',
    date: 'Nov 2021',
    description:
      'Web creada para el cliente Pasión por las Finanzas, empresa del sector financiero, donde se muestra información de la empresa, servicios, blog y contacto. Se utilizó Wordpress como CMS y ReactJS para el desarrollo del tema. Además de un apartado de reserva integrado con la pasarela de pago Izipay.',
    image: 'pf.webp',
    tags: [
      'ReactJS',
      'NextJS',
      'Material UI',
      'HTML',
      'CSS',
      'JavaScript',
      'Wordpress',
    ],
    category: 'web app',
    webapp: 'https://pasionporlasfinanzas.com',
  },
  {
    id: 3,
    title: 'Transportes y Servicios Valso',
    date: 'Ago 2022',
    description:
      'Web creada para el cliente con el fin de aumentar su exposición, logrando obtener un aumento de afilidos y clientes de un 30% según lo manifestado por la empresa.',
    image: '/valso.webp',
    tags: ['Wordpress', 'HTML', 'CSS', 'JavaScript'],
    category: 'web app',
    webapp: 'https://transportesvalso.com',
  },
  {
    id: 4,
    title: 'Sistema de Ventas',
    date: 'Oct 2020',
    description:
      'Creación de BD, registro de productos, realización de ventas y generación de informes. Se utilizó Java como lenguaje de programación, MySQL como gestor de base de datos y Netbeans como IDE de desarrollo. Además, se utilizó el patrón de diseño MVC para la arquitectura del proyecto.',
    image: '/sistema_ventas.png',
    tags: ['Java', 'MVC', 'MySQL', 'Netbeans', 'Jasper Report'],
    category: 'desktop app',
    github: 'https://github.com/tvalverdes/minimarketchelita',
  },
]
